...
import { Accounts, type AccountListResponse, type AccountMarketingGetResponse, type AccountMarketingUpdateResponse, type AccountListParams, type AccountUpdateParams, type AccountMarketingUpdateParams, type AccountOrdersGetParams } from "./resources/accounts";
...
export interface ClientOptions {
  /**
   * Use the storefront public key as the bearer token in the Authorization header.
   */
  storefrontPublicKey?: string | AuthTokenProvider | undefined;
...
export type PuddleStorefrontAPIOptions = ClientOptions;

/**
 * API Client for interfacing with the PuddleStorefrontApi API.
 */
export class PuddleStorefrontAPI {
  storefrontPublicKey: string | AuthTokenProvider | undefined;
  ...
  constructor({
    ...
    const defaultBaseURL = "https://api.puddle.co.za/storefront/v1";
    this.baseURL = options.baseURL || defaultBaseURL;
    this.timeout = options.timeout ?? PuddleStorefrontAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.storefrontPublicKey = storefrontPublicKey;
  }
  ...
  withOptions(options: Partial<ClientOptions>): this {
    const client = new (this.constructor as new (props: ClientOptions) => this)({
      ...
      storefrontPublicKey: this.storefrontPublicKey,
      ...options,
    });
    return client;
  }
  ...
  authHeadersSync(): Record<string, string> {
    ...
    const storefrontPublicKey = this.resolveAuthOptionSync("storefrontPublicKey", this.storefrontPublicKey);
    if (storefrontPublicKey) headers['Authorization'] = `Bearer ${storefrontPublicKey}`;
    return headers;
  }

  webSocketAuthHeaders(): Record<string, string> {
    const storefrontPublicKey = this.resolveAuthOptionSync("storefrontPublicKey", this.storefrontPublicKey);
    if (storefrontPublicKey) return { Authorization: `Bearer ${storefrontPublicKey}` };
    return {};
  }
  ...
  private async authHeadersAsync(): Promise<Record<string, string>> {
    const headers: Record<string, string> = {};
    const storefrontPublicKey = await this.resolveAuthOption("storefrontPublicKey", this.storefrontPublicKey);
    if (storefrontPublicKey) headers['Authorization'] = `Bearer ${storefrontPublicKey}`;
    return headers;
  }
  ...
  private async resolveAuthOption(optionName: string, value: string | AuthTokenProvider | null | undefined): Promise<string | undefined> {
    ...
    if (!token) throw new Errors.PuddleStorefrontAPIError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  private resolveAuthOptionSync(optionName: string, value: string | AuthTokenProvider | null | undefined): string | undefined {
    ...
    if (typeof token !== "string" || !token) throw new Errors.PuddleStorefrontAPIError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }

  static PuddleStorefrontAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PuddleStorefrontAPIError = Errors.PuddleStorefrontAPIError;
...
PuddleStorefrontAPI.Accounts = Accounts;
PuddleStorefrontAPI.Auth = Auth;
PuddleStorefrontAPI.Cart = Cart;
PuddleStorefrontAPI.Collections = Collections;
PuddleStorefrontAPI.Products = Products;
PuddleStorefrontAPI.Content = Content;
PuddleStorefrontAPI.Wishlist = Wishlist;

export declare namespace PuddleStorefrontAPI {
